...
class Accountledgers_Model extends MY_Model {
	...
	public function get_accountledger_list($school_id = null,$financial_year_id = null,$category =null){        
        $this->db->select('AL.*,ALD.opening_balance,ALD.opening_cr_dr,ALD.budget,ALD.budget_cr_dr, S.school_name, AG.name as group_name,AG.group_code as group_code,AT.name as type_name,AY.session_year');
...
/*				$final_amount=$opening_balance+$grand_total;
	...
	public function get_effective_balance_by_ledger($ledger_id,$opening_balance=0,$opening_cr_dr='DR',$date=null,$start_date=null,$end_date=null,$voucher_category=null,$no_financial_year=null){
...
//print_r($transactions);print 'next';		
	...
	public function get_accountledgers_by_group($school_id = null,$financial_year_id = null,$group_id){        
        $this->db->select('AL.*,ALD.opening_balance,ALD.opening_cr_dr,ALD.budget,ALD.budget_cr_dr, S.school_name, AG.name as group_name,AT.name as type_name');
        $this->db->from('account_ledgers AS AL');       		
        $this->db->join('schools AS S', 'S.id = AL.school_id', 'left');
		$this->db->join('account_groups AS AG', 'AG.id = AL.account_group_id', 'left');
...
//			$detail_arr['academic_year_id']=$academic_year_id;
	...
	public function delete_all($school_id){
		if($school_id)
		{	$this->db->query("delete from account_ledger_details where ledger_id in ( SELECT id FROM `account_ledgers` where school_id= $school_id )");	
			$this->db->query("delete  FROM `account_ledgers` where school_id= $school_id ");	
		}
	}
